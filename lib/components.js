...
var button = ({ children, type = 'text', icon, className = '', ...rest }) => (React.createElement("button", Object.assign({ className: `${className} ${style.container} ${style[type]}` }, rest), children));
...
class Input extends React.Component {
    ...
    render() {
        ...
        return (React.createElement("div", { className: classnames(style$1.container, { [style$1.password]: type === 'password' }, this.handleValidationClass()) },
            label ? React.createElement("div", { className: style$1.label }, label) : null,
            React.createElement("div", { className: `${style$1.inputWrapper} ${icon ? icon : ''}` },
                React.createElement(Type, Object.assign({ value: value, onChange: e => onChange(e.target.value), onKeyPress: e => onSubmit && e.key === 'Enter' ? onSubmit() : undefined, className: classnames(style$1.input, className), onBlur: () => onValidate && this.setState({ validate: true }), type: type }, args))),
            caption ? React.createElement("div", { className: style$1.caption }, caption) : null));
    }
}
